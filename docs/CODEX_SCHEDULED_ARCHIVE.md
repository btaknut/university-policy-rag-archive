...
9. Gate가 성공하면 데이터 초안 PR을 생성한다. Gate 실패 시 데이터 PR을 만들지 않는다.
10. 저장소 관리자가 데이터 PR의 원본·변환물·메타데이터를 최종 검토하고 병합한다.
...
| 최종 승인 | 저장소 관리자 | 데이터 PR 병합 |
| 최종 확인 | Codex 예약 작업 | 최신 `main` 검증 및 결과 보고 |

## 자동 반영 조건

다음 조건을 모두 충족해야 데이터 초안 PR을 자동 생성한다.
...
하나라도 충족하지 않거나 상태를 확인할 수 없으면 데이터 초안 PR을 생성하지 않는다.
...
- `mode=create_pr`: Gate 통과 후 데이터 초안 PR을 생성한다.
