...
`is_latest_version`은 가장 최근 확보한 파일, `validity_status`는 공식 근거로 확인한 시행 효력을 뜻한다. 두 축은 독립적이다. 청크는 `section_kind`, `retrieval_scope`, `normative_status`로 규범 본문과 개정 참고자료를 분리한다. 기존 ID를 유지한 데이터 보강과 청크 재생성은 코드 PR 이후 별도 데이터 PR에서 수행한다.
