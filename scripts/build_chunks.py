...
def divide_long(text: str, target: int = 700, maximum: int = 1200) -> list[str]:
    ...
    for version in versions:
        ...
        for article_index, (article_no, article_title, article_text) in enumerate(split_article(text)):
            for subindex, piece in enumerate(divide_long(article_text)):
                idx = len([c for c in chunks if c["version_id"] == version["version_id"]]); location = article_no or f"section-{article_index+1}"; chunk_id = f"CHK-{version['version_id']}-{re.sub(r'[^0-9A-Za-z가-힣-]', '', location)}-{subindex+1:03d}"
