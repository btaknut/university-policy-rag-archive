...
def main() -> int:
    versions = read_jsonl(ROOT / "metadata/versions.jsonl"); groups = defaultdict(list); conflicts = []
    ...
    write_jsonl(ROOT / "metadata/versions.jsonl", versions); write_csv(ROOT / "metadata/conflicts.csv", conflicts, CONFLICT_FIELDS)
